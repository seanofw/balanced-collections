...
namespace BalancedCollections.Tests.RedBlackTree
{
	...
	public class WeightedRedBlackTreeTests
	{
		/// <summary>
		...
		public void NextShouldBeAbleToWalkTheTreeForward()
		{
			...
			for (int i = 0; i < 500; i++)
			{
				...
				if (IsFibonacci(i))
				{
					...
					for (WeightedRedBlackTreeNode<int, int> node = tree.MinimumNode; node != null; node = node.Next())
					{
		...
		{
			WeightedRedBlackTree<Bar, int> tree = new WeightedRedBlackTree<Bar, int>
			{
				{ new Bar(1), 10 },
				{ new Bar(3), 20 },
				{ new Bar(5), 30 },
		...
		{
			Assert.Throws<ArgumentException>(() => new WeightedRedBlackTree<Baz, int>());
		}
		...
		public void CanCopyTreesByNode()
		{
			...
			WeightedRedBlackTree<int, int> newTree = new WeightedRedBlackTree<int, int>
			{
				tree.Find(9),
				tree.Find(7),
				tree.Find(5),
		...
		public void CanFindGreatestUnderALimit()
		{
			...
			for (int i = 0; i < 500; i++)
			{
				...
				if ((i % 20) == 19)
				{
					...
					WeightedRedBlackTreeNode<int, int> node = tree.GreatestBefore(key - 1);
					Assert.That(node.Key, Is.EqualTo(valueUnderLastKey));

					int valueUnderLastKey2 = keys.OrderBy(k => k).Last(k => k < key);
					WeightedRedBlackTreeNode<int, int> node2 = tree.GreatestBefore(key);
		...
		public void CanFindGreatestAtOrUnderALimit()
		{
			...
			for (int i = 0; i < 500; i++)
			{
				...
				{
					int valueUnderLastKey = keys.OrderBy(k => k).Last(k => k <= key - 1);
					WeightedRedBlackTreeNode<int, int> node = tree.GreatestBeforeOrEqualTo(key - 1);
					Assert.That(node.Key, Is.EqualTo(valueUnderLastKey));

					int valueUnderLastKey2 = keys.OrderBy(k => k).Last(k => k <= key);
					WeightedRedBlackTreeNode<int, int> node2 = tree.GreatestBeforeOrEqualTo(key);
					Assert.That(node2.Key, Is.EqualTo(valueUnderLastKey2));
				}
			}
		}
		...
		public void CanFindLowestAboveALimit()
		{
			...
			for (int i = 0; i < 500; i++)
			{
				...
				if ((i % 20) == 19)
				{
					...
					WeightedRedBlackTreeNode<int, int> node = tree.LeastAfter(key + 1);
					Assert.That(node.Key, Is.EqualTo(valueUnderFirstKey));

					int valueUnderFirstKey2 = keys.OrderBy(k => k).First(k => k > key);
					WeightedRedBlackTreeNode<int, int> node2 = tree.LeastAfter(key);
		...
		public void CanFindLowestAtOrAboveALimit()
		{
			...
			for (int i = 0; i < 500; i++)
			{
				...
				{
					int valueUnderFirstKey = keys.OrderBy(k => k).First(k => k >= key + 1);
					WeightedRedBlackTreeNode<int, int> node = tree.LeastAfterOrEqualTo(key + 1);
					Assert.That(node.Key, Is.EqualTo(valueUnderFirstKey));

					int valueUnderFirstKey2 = keys.OrderBy(k => k).First(k => k >= key);
					WeightedRedBlackTreeNode<int, int> node2 = tree.LeastAfterOrEqualTo(key);
					Assert.That(node2.Key, Is.EqualTo(valueUnderFirstKey2));
				}
			}
		}
		...
		public void CanFindLowestInARange()
		{
			...
			for (int i = 0; i < 500; i++)
			{
				...
				if ((i % 20) == 19)
				{
					...
					WeightedRedBlackTreeNode<int, int> node;
		...
		public void CanEnumerateNewSchool()
		{
			...
			IEnumerator<WeightedRedBlackTreeNode<int, int>> enumerator = ((IEnumerable<WeightedRedBlackTreeNode<int, int>>)tree).GetEnumerator();
			...
			enumerator = ((IEnumerable<WeightedRedBlackTreeNode<int, int>>)tree).GetEnumerator();
			int index = 0;
		...
		public void CanEnumerateOldSchool()
		{
			...
			while (enumerator.MoveNext())
			{
				Assert.That(((WeightedRedBlackTreeNode<int, int>)enumerator.Current).Key,
		...
		{
			int seed = 12345;
			WeightedRedBlackTree<int, int> tree;
			List<int> keys;
			...
			{
				seed = 12345;
				tree = new WeightedRedBlackTree<int, int>();
				keys = new List<int>();
		...
		public void CanTestForNodeContains()
		{
			...
			Assert.That(tree.Contains(new WeightedRedBlackTreeNode<int, int>(5, 5)), Is.False);

			WeightedRedBlackTreeNode<int, int> node = tree.MinimumNode;
		...
		public void CanExplicitlyRemoveKnownNodes()
		{
			...
			Assert.That(tree.Remove(new WeightedRedBlackTreeNode<int, int>(5, 5)), Is.False);
